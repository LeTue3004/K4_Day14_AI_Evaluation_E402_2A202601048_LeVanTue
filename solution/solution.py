...
class RAGASEvaluator:
    ...
    def run_full_eval(
        ...
        pair = QAPair(question=question, expected_answer=expected, context=context)
        ...
        return EvalResult(pair, answer, faithfulness, relevance, completeness, passed,
                          failure_type, precision, recall)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the response from 0 to 1 for every rubric criterion. Return a JSON "
            "object mapping criterion names to numeric scores.\n\n"
            f"Question: {question}\nAnswer: {answer}\nRubric:\n{rubric_text}"
        )
        ...
        try:
            ...
            scores = {
                criterion: min(1.0, max(0.0, float(parsed.get(criterion, 0.5))))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        all_scores = [float(score) for item in scores_batch for score in item.get("scores", {}).values()]
        average = sum(all_scores) / len(all_scores) if all_scores else 0.0
        first_scores = [item.get("scores", {}).get("first_response") for item in scores_batch]
        second_scores = [item.get("scores", {}).get("second_response") for item in scores_batch]
        paired = [(float(first), float(second)) for first, second in zip(first_scores, second_scores)
                  if first is not None and second is not None]
        positional = bool(paired) and sum(first > second for first, second in paired) / len(paired) > 0.7
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            "total": total, "passed": sum(result.passed for result in results),
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def average(items: list, attribute: str) -> float:
            return sum(getattr(item, attribute) for item in items) / len(items) if items else 0.0
        metrics = ("faithfulness", "relevance", "completeness")
        report = {f"new_avg_{metric}": average(new_results, metric) for metric in metrics}
        report.update({f"baseline_avg_{metric}": average(baseline_results, metric) for metric in metrics})
        regressions = [metric for metric in metrics if report[f"baseline_avg_{metric}"] - report[f"new_avg_{metric}"] > 0.05]
    ...
    def identify_failures(
        ...
        """
        return [result for result in results if any(score < threshold for score in
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Investigate failure trace"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append("Add grounding checks and require claims to be supported by retrieved context.")
        if categories.get("irrelevant") or categories.get("off_topic"):
            suggestions.append("Improve intent routing and add prompt examples that directly answer the user question.")
        if categories.get("incomplete"):
            suggestions.append("Retrieve more relevant evidence and use an answer checklist for conditions and exceptions.")
        defaults = ["Review failed traces and add representative cases to the golden dataset.",
                    "Tune retrieval chunking, ranking, and top-k using the failure clusters.",
                    "Run regression evaluation before deploying the next change."]
